...
export async function getHomeVideo() {
    const query = '*[_type == "home"][0]{videoFile{asset->{url, originalFilename}}}'
    const data = await sanityClient.fetch(query);
    return data?.videoFile?.asset?.url || null;
}

export async function getHomeHero() {
    const query = '*[_type == "home"][0]{videoFile{asset->{url, originalFilename}}, posterImage}';
    const data = await sanityClient.fetch(query);
    const videoUrl = data?.videoFile?.asset?.url || null;
    let posterUrl = null;
    if (data?.posterImage) {
        try { posterUrl = urlFor(data.posterImage).url(); } catch (e) { posterUrl = null; }
    }
